...
def run_module():
    ...
    device = {}
    device = viptela.get_device_by_uuid(viptela.params['uuid'], type='vedges')
    ...
    if viptela.params['state'] == 'cert':
        ...
        if 'vedgeCSR' in device:
            viptela.result['deviceCSR'] = device['vedgeCSR']
        else:
            viptela.result['what_changed'].append('csr')
            if not module.check_mode:
                if 'deviceIP' in device:
                    viptela.result['deviceCSR'] = viptela.generate_csr_hw_vedge(device['deviceIP'])
                else:
